...
function _buildSheetSteps() {
  return [
    ...
    {
      // DISC-3: знак раскрытия. Цель — первая строка характеристики; она есть
      // на листе всегда, но в «классической» раскладке ромба у неё нет —
      // отсюда requireTarget по селектору, который живёт только в 2024.
      requireTarget: true,
      target: function () {
        return _tourFirstVisible(['html[data-stats-layout="2024"] #stat-block-str .abil-card-head']);
      },
      title: 'Ромб слева — есть продолжение',
      text: 'Половина ромба закрашена — строка раскрывается по нажатию: внутри навыки, броски и описание. Когда раскроете, заливка ляжет вниз. Пустой ромб значит, что раскрывать нечего.',
      novice: 'Все знаки собраны в «Справке» → «Условные знаки».'
    },
    {
      // E24-16: панель предыстории 2024 видна только у персонажа 2024 после
      // фиксации основы — у 2014 шаг выпадает сам.
      requireTarget: true,
      target: function () {
        return _tourFirstVisible(['#bg-extras-panel']);
      },
      title: 'Происхождение 2024',
      text: 'В редакции 2024 характеристики повышает предыстория: распределите +2/+1 или +1/+1/+1 между её тремя характеристиками. Здесь же черта происхождения и стартовое снаряжение.',
      novice: 'Вид в 2024 не даёт прибавок к характеристикам — только размер, скорость и видовые черты.'
    },
    {
      requireTarget: true,
      target: function () {
      ...
      novice: 'Отключить механизм можно в «Настройки → Замок листа».'
    },
    {
...
function _buildSpellsSteps() {
  return [
    ...
    },
    {
      ...
      title: 'Ячейки заклинаний',
      text: 'Ромбы по уровням — ваши ячейки. Нажатие отмечает потраченную; счётчик показывает свободные/всего. «Длинный отдых» восстанавливает все ячейки.',
    ...
    },
    {
      ...
      text: '«Найти заклинание» — добавить из встроенной базы с фильтрами по классу и уровню. «Добавить своё» — вписать собственное заклинание вручную.'
    },
    {
      title: 'Готово',
...
function _buildInventorySteps() {
  return [
    ...
    },
    {
      ...
      title: 'Слоты и переноска',
      text: 'Сколько вы несёте: «занято / всего» слотов и полоса заполнения. При превышении предела появляется «Перегруз».',
...
function _buildPartySteps() {
  return [
    ...
    },
    {
      ...
      novice: 'Сердце — текущие/максимальные хиты, щит — класс доспеха (насколько трудно по вам попасть). Меняются на вкладке «Бой» и на листе; тут только сводка.'
    },
    {
      requireTarget: true,
...
function _buildJournalSteps() {
  return [
    ...
    },
    {
      ...
      title: 'Фильтры',
      text: 'Кнопки оставляют в списке только нужный тип записей: повышения уровня, отдых, изменения статов, черты или заметки. «Все» снимает фильтр — там же видны добавленные вручную события без своей кнопки (бой, сюжет, добыча, смерть).'
    },
    {
      title: 'Готово',
