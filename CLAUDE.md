...
- `spell-effects.js` — механика применения заклинаний (план CAST): кураторская таблица `SPELL_EFFECTS` (ключ = имя заклинания, оверрайды редакций `bySource`, выбор при касте — `variants`) + чистые хелперы `getSpellEffect`/`scaleFormula`/`damageFormulaFor`/`critFormula`/`buildCompanionPrefill`; потребитель — `applyCastEffects` в `app-spells.js` (урон в цель боя — `offerCastDamageToBattle` в `app-party.js`).
