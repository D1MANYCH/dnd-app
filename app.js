...
(function() {
  ...
  function isInteractive(el) {
    if (!el) return false;
    // Не перехватываем свайпы на интерактивных/прокручиваемых элементах
    var tag = (el.tagName || "").toLowerCase();
    if (tag === "input" || tag === "textarea" || tag === "select" || tag === "button") return true;
    if (el.isContentEditable) return true;
    // Поднимаемся по дереву, ищем горизонтально-скроллящихся предков и слайдеры
    var node = el;
  ...
  document.addEventListener("touchend", function(e) {
    ...
    if (dt > 600) return; // слишком медленный жест — игнорируем
    ...
    //    и не на интерактивном элементе/горизонтальном скролле.
