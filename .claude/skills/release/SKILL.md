...
6. Дождаться CI: `gh run list --limit 2` (оба workflow — `tests` и `pages` — должны быть success).
   `gh` установлен, но **не в PATH**: полный путь `C:\Program Files\GitHub CLI\gh.exe`.
7. **Пост-анонс — обязательный финал релиза, отдавать не дожидаясь просьбы.**
   `node tools/gen-release-post.js [версия] [--out <slug>]` собирает скелет с тремя ссылками —
   патч, подробный лог, сайт. Тело переписать руками: 2–3 предложения, сухо по факту,
   тон = changelog (числа/имена/значения, без эпитетов, рекламных вступлений и имён функций);
   футер начинается ссылкой на полный патч, ссылки на CHANGELOG.md в посте нет.
   Готовый текст показать в чате блоком — публикует пользователь сам.
   `--out` кладёт файл в `docs/marketing/posts/NN-*.txt`; каталог в .gitignore, коммитить нечего.

Тот же финал у `/ship` — расхождения между двумя путями быть не должно: релиз считается
законченным, когда есть зелёный CI **и** текст поста.
